...
> **Bridge**：A/B pair 只是结构正确，saliency 仍不能定义在非唯一的 factor space  
> **Jump 2**：即使原子正确，magnitude-only selection 导致跨客户端选择高度集中 → 需要 signal-noise-aware allocation  
> 两跳与桥段之后自然过渡到方法：$P_1$ 做 module-level quota allocation，$P_2$ 做 client-level complementary assignment。

---

## 0. 核心叙事一句话

> Low-budget sparse FedLoRA upload cannot treat LoRA tensors as independent compression units, cannot define saliency in the non-unique LoRA factor space, and cannot rank effective updates purely by magnitude. The correct approach is to preserve shared descent signals under client heterogeneity through signal-noise-aware allocation.
...
本文通过两个实验现象和一个简短理论桥段逐层揭示低预算 FedLoRA sparse upload 的核心挑战。
...
这一步解决的是 **structural consistency**：上传单元必须保持 A/B factors 的乘法结构闭合。它还没有完全回答 saliency 应该如何定义，因为即使上传单元是完整 A/B pair，仍然可以按 factor-space norm 选择 pair。

### 2.4 Bridge：A/B pair 解决结构问题，但 factor norm 不是内在重要性

A/B pair atomic upload 之后，一个自然选择是按 factor-space norm 排序：

$$
\|U_{A,i,m}\|_F^2+\|U_{B,i,m}\|_F^2.
$$

这个做法满足结构一致性，但 saliency 仍然依赖 LoRA factor 的具体表示。LoRA factorization 本身不是唯一的：对任意可逆矩阵 $S\in\mathbb{R}^{r\times r}$，有

$$
B_mA_m=(B_mS)(S^{-1}A_m).
$$

因此，同一个 effective update 可以对应不同的 factor 表示，而 factor norm 会随 reparameterization 改变。换言之，$\|U_A\|_F^2+\|U_B\|_F^2$ 不是 effective model change 的内在属性。若一个上传策略会因为等价 factor 表示改变而改变选择结果，它就不具备 representation consistency。

这说明 A/B atomicity 只是必要条件。为了让 saliency 与实际模型更新一致，重要性应定义在 A/B pair induced effective-update space 中，即比较 $\Phi_{i,m}$，而不是比较原始 factor norm。于是，最自然的下一步是考察 effective-update magnitude：
...
Jump 1 和上述 bridge 确定了两个基本要求：上传单元应该是完整 A/B pair，saliency 应该定义在 A/B pair induced effective-update space 中。下一个问题是：如何度量每个 $\Phi_{i,m}$ 的"重要性"？
...
这里的统计量来自每轮 sparse upload 的 selection record。对第 $t$ 轮，记 client $i$ 选择的 modules 集合为 $\mathcal{S}_i^t$：

- **Pairwise Jaccard**：对同一轮内所有 client pairs 计算 $|\mathcal{S}_i^t\cap\mathcal{S}_j^t|/|\mathcal{S}_i^t\cup\mathcal{S}_j^t|$ 后取平均。Jaccard 越接近 1，说明不同 clients 选择越相同；Jaccard = 1 表示所有 clients 选择完全相同的 module set。
- **Union Selected**：$\left|\cup_i\mathcal{S}_i^t\right|$，表示这一轮所有 clients 合起来覆盖了多少个不同 modules。
- **Fully Shared Modules**：$\left|\cap_i\mathcal{S}_i^t\right|$，表示有多少 modules 被本轮所有 clients 同时选择。该值越大，说明重复上传越严重。

| Round | AB-Effective Jaccard | AB-Factor Jaccard | **Ours Jaccard** | AB-Effective Union | AB-Factor Union | **Ours Union** |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 0.82 | 0.74 | **0.19** | 24 | 30 | **80** |
| 2 | **1.00** | 0.84 | **0.17** | 20 | 34 | **80** |
| 3 | **1.00** | 0.60 | **0.18** | 20 | 32 | **80** |
| 4 | **1.00** | 0.55 | **0.17** | 20 | 44 | **84** |
| 5 | **1.00** | 0.43 | **0.16** | 20 | 52 | **84** |
| **Mean** | **0.96** | 0.63 | **0.18** | 20.8 | 38.4 | **81.6** |

| Round | AB-Effective Fully Shared | AB-Factor Fully Shared | **Ours Fully Shared** |
|---:|---:|---:|---:|
| 1 | 10 | 10 | **0** |
| 2 | 20 | 12 | **0** |
| 3 | 20 | 8 | **0** |
...
- AB-Factor 也存在明显集中（Jaccard 0.43-0.84），但不像 AB-Effective 那样完全 collapse：union 从 30 增至 52，fully shared 从 10 降至 6。
...
| Llama-3.1-8B | 17.46 | 6.44 | 5.53 | **15.72** | +9.27 | 84.2% |
...
### Requirement 1：有效的上传原子与 saliency 空间
上传单元必须是完整 A/B pair，避免单独上传 $U_A$ 或 $U_B$ 造成 structural error。同时，saliency 不应定义在非唯一的 factor space，而应定义在 A/B pair induced effective-update space $\Phi_{i,m}$ 中，从而满足 representation consistency。

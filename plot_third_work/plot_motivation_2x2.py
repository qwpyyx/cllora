...
def panel_gsm8k(ax):
    ...
    ax.annotate("Catastrophic\nfailure",
                xy=(0, 2.2), xytext=(0, 11.5),
                arrowprops=dict(arrowstyle="->", color=C["red"], lw=0.9),
                fontsize=7.3, color=C["red"], fontweight="bold", ha="center")
    ax.annotate("A/B pair recovers\n+26.2 EM",
                xy=(1, 30.0), xytext=(1, 37.0),
                arrowprops=dict(arrowstyle="->", color=C["blue"], lw=0.9),
                fontsize=7.2, color=C["blue"], fontweight="bold", ha="center")
...
def panel_overlap(ax):
    ...
    ax.text(3.05, 0.91, "Jaccard=1:\nsame modules",
            color=C["red"], fontsize=7.2, ha="center", va="center")
    ax.text(3.00, 0.25, "low cross-client overlap",
            color=C["green"], fontsize=7.2, ha="center", va="center")
...
def panel_coverage(ax):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["AB-Effective", "AB-Factor", "Ours"])
    ax.set_ylim(0, 105)
    ax.yaxis.set_major_locator(mticker.MultipleLocator(20))
    ax.grid(axis="y", alpha=0.22, linestyle="--", linewidth=0.7)
...
def make_figure():
    ...
    # Height is chosen so each panel has comparable aspect ratio.
    ...
    fig.set_constrained_layout_pads(w_pad=0.02, h_pad=0.02, wspace=0.08, hspace=0.08)

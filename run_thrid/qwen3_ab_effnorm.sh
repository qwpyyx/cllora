#!/bin/bash
# Test: ab_pair + effective_norm (structurally correct atom + representation-consistent saliency, no SN)
...
CUDA_VISIBLE_DEVICES=0,1,2,3 ${PY} -m accelerate.commands.launch \
  ...
  --run_name ab_effnorm --max_source_length 512 --max_target_length 50 --generation_max_length 50 \
  ...
  --lamda_1 0 --lamda_2 0 --federated_seed 28 --method lora_origin --task 1 --radius 0 \

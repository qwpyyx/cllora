#!/bin/bash
# FLM-TopK at alpha=1: how much does heterogeneity hurt magnitude-only methods?
...
P=$(shuf -i25000-30000 -n1)
CUDA_VISIBLE_DEVICES=0,1,2,3 ${PY} -m accelerate.commands.launch \
  --config_file script_accelerate/accelerate_config.yaml --main_process_port ${P} \
  ...
  --output_dir results/Qwen3_flmtopk_a1_seed28 \
  --per_device_train_batch_size 16 --per_device_eval_batch_size 16 \
  --gradient_accumulation_steps 2 --global_rounds 5 --local_epochs 10 \
  --num_clients 50 --clients_per_round 10 --dirichlet_alpha 1 \
  --partition_strategy quantity --comm_budget 1760 --learning_rate 1e-04 \
  --run_name flmtopk_a1 --max_source_length 512 --max_target_length 50 --generation_max_length 50 \
  --add_task_name False --add_dataset_name False --overwrite_output_dir --overwrite_cache \
  --lr_scheduler_type constant --warmup_steps 0 --logging_strategy steps --logging_steps 2 \
  --evaluation_strategy no --save_strategy no --save_steps 150 \
  --lamda_1 0 --lamda_2 0 --federated_seed 28 --method flm_topk --task 1 --radius 0 \
  --gradient_checkpointing True --bf16 True --ddp_find_unused_parameters False \
  ...
  > results/Qwen3_flmtopk_a1_seed28.log 2>&1
echo EXIT=$?

#!/bin/bash
# Ours batch3: test qv_factor_norm + encoding with larger m values
...
OUT=results/Qwen3_ours_batch3; rm -rf ${OUT}; mkdir -p ${OUT}/logs

# Test 1: qv_factor_norm + ComPEFT m5 (larger candidate pool)
P1=$(shuf -i25000-30000 -n1)
CUDA_VISIBLE_DEVICES=0,1,2,3 ${ACC} --main_process_port ${P1} src/run_uie_lora.py \
  ${C} --output_dir ${OUT}/qv_m5_seed28 --run_name b3_qv_m5 \
  --upload_atomic_mode qv_block --upload_score_mode factor_norm \
  --sn_encoder_mode compeft --sn_candidate_budget_multiplier 5 \
  --sn_encoder_packet_num 1760 \
  > ${OUT}/logs/qv_m5.log 2>&1 &
PID1=$!

# Test 2: qv_factor_norm + ComPEFT m6
P2=$(shuf -i25000-30000 -n1)
CUDA_VISIBLE_DEVICES=4,5,6,7 ${ACC} --main_process_port ${P2} src/run_uie_lora.py \
  ${C} --output_dir ${OUT}/qv_m6_seed28 --run_name b3_qv_m6 \
  --upload_atomic_mode qv_block --upload_score_mode factor_norm \
  --sn_encoder_mode compeft --sn_candidate_budget_multiplier 6 \
  --sn_encoder_packet_num 1760 \
  > ${OUT}/logs/qv_m6.log 2>&1 &
PID2=$!

wait ${PID1}; wait ${PID2}
echo "=== Results ==="
for t in qv_m5 qv_m6; do

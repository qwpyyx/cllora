...
class ContinualState:
    ...
    bar_B: Dict[str, torch.Tensor] = field(default_factory=dict)  # 历史Fisher-参数乘积（已归一化）
    gamma: float = 1.0
    ...
    def save(self, path: str) -> None:
        torch.save({'bar_F': self.bar_F, 'bar_B': self.bar_B, 'eta_history': self.eta_history}, path)

...
# ============================================================
# 2 x 4-GPU parallel budget sweep for Dolly pair-saliency diag
#   Group A uses GPU 0,1,2,3
#   Group B uses GPU 4,5,6,7
# Each group runs its assigned budgets sequentially.1
# ============================================================
...
# Split 6 budgets into two 4-GPU jobs.
# Interleaving avoids putting all small/large budgets in the same group.
BUDGETS_A=(440 1320 2200)
BUDGETS_B=(880 1760 3080)
...
base_out=results/Qwen2_dolly/lora_exp_dolly_alpha05/pairdiag_budget_sweep_seed${seed}_2x4gpu
...
run_one_budget() {
    ...
    echo "============================================"
    echo "[${group_name}] Running pair-saliency diagnostics: budget=${com_budget}, seed=${seed}, GPUs=${cuda_devices}"
    echo "============================================"

    local port
    port=$(shuf -i25000-30000 -n1)
    local out_dir=${base_out}/budget_${com_budget}_abpair_pairdiag_5client
    local log_file=${base_out}/logs/${group_name}_budget_${com_budget}.log
    ...
    CUDA_VISIBLE_DEVICES=${cuda_devices} accelerate launch --config_file ${ACCEL_CONFIG} \
       ...
       ${random_layer_param} 2>&1 | tee ${log_file}
...
run_group() {
    ...
    for com_budget in "${budgets[@]}"; do
        run_one_budget "${com_budget}" "${cuda_devices}" "${group_name}"
    done

    echo "[${group_name}] finished all budgets: ${budgets[*]}"
}

# Run two 4-GPU groups in parallel.
run_group groupA 0,1,2,3 "${BUDGETS_A[@]}" &
pid_a=$!

run_group groupB 4,5,6,7 "${BUDGETS_B[@]}" &
pid_b=$!

wait ${pid_a}
wait ${pid_b}

echo "All 2x4-GPU pair-saliency budget-sweep diagnostics finished."
